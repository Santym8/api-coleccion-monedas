...
def download_img(img_url, img_name):
    url_save = f'api/img/50 State quarters program/{img_name}.jpg'
    req = requests.get(img_url)
    response = req.content
    file = open(url_save, "wb")
    file.write(response)
    file.close()
    return  f'img/50 State quarters program/{img_name}.jpg'

url = 'https://www.usmint.gov/learn/kids/about-the-mint/50-state-quarters'

page_response = requests.get(url, timeout=5)
page_content = BeautifulSoup(page_response.content, "html.parser")

coins = []
divs = page_content.find_all('div',class_= "feature-group")
count = 1
for div in divs:
...
with open('Scripts/json/states.json', 'w') as outfile:
